export default (services) => () => {
  ...
  return {
    ...
    getPredictionsByStpid: (stpid) => {
      return api.get(`/getpredictions?key=${process.env.API_KEY}&stpid=${stpid}`)
        .then(function (res) {
          return XMLtoJSON(res.data);
        })
        .then(data => {
          let prd = data['bustime-response'].prd;
          if(! prd) prd = [];
          prd = prd.map(x => stripXMLOBJ(x));
          prd = prd.map(x => { // convert time to unix
            x.tmstmp = services.moment(x.tmstmp, 'YYYYMMDD HH:mm:ss').unix();
            x.prdtm = services.moment(x.prdtm, 'YYYYMMDD HH:mm:ss').unix();
            x.vid = x.vid.length > 1 ? x.vid : null;
            return x;
          });
          return prd;
        });
